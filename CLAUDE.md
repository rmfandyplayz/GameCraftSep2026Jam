...
Assets/Scripts/UI/Utility/    small self-contained UI utilities, one folder each
  ...
  FlipnoteStyleAnimation/     sprite-frame flipbooks for UGUI Images + its README.md
...
**Reversing is an authoring operation, not a playback mode.** The `Mirror Animation` / `Duplicate as Mirrored` right-click commands rewrite the authored data once so you get a real second animation to tune — values, `SetActive` and staggered delays all invert; punch/shake and `PlaySound` pass through. **Easing is deliberately not mirrored** (neither the `Ease` preset nor a custom curve): the house style is that things decelerate into place in both directions, so a mirrored `Hide` keeps its `Show`'s `Out` ease. That was removed on request after being built — don't reintroduce it. Nothing at runtime knows about mirroring, which is why the whole thing lives in `Editor/UIAnimationMirror.cs` and the runtime has no `PlayReverse`. **Do not add one back** without asking: a live reverse mode and an authored mirror are two answers to the same question, and having both was tried and cut.
...
**Edit-mode preview writes to real scene objects, so it is bracketed by capture/restore, not by hope.** `UIAnimationPlayerEditor` drives the sequence off `DG.DOTweenEditor.DOTweenEditorPreview` when not in play mode. Three things make that safe and all three are load-bearing: `UIAnimationPlayer.EditorPrepareForPreview` re-resolves targets and re-captures baselines **before every preview** (Awake never runs in edit mode, and re-capturing from the restored state is what stops `To: Baseline` endpoints drifting on repeat previews); `UIAnimationStep.RestoreBaseline` writes only the one property that step drives, because a blanket `EditorJsonUtility` round-trip of the component would also blank object references like an Image's sprite; and callbacks are cleared so an `On Complete` UnityEvent cannot run game code outside play mode. `PlaySound` is suppressed via `UIAnimationStep.EditorSuppressSound`.
...
- **Enums that appear on serialized fields are stored as integers, and those integers are the contract.** `UIAnimationStepType`, `UIAnimationEndpointMode` and `UIAnimationStartMode` now assign their values **explicitly** (`Scale = 12`), which makes reordering the lines harmless — a value carries its number with it. *Changing* a number or reusing a retired one still silently repoints every asset authored against it, with no error and no warning. **This has already cost this project once**, back when the numbers were implicit: `7e7af24` alphabetised `UIAnimationStepType`, which turned the 16 authored `Scale` steps on the main-menu buttons (`Type: 2`) into `GraphicAlpha` steps that faded the buttons to invisible; repaired in the prefab by rewriting those 16 to `Type: 12`. The symptom looks like lost references or a bad prefab conversion, so if animations "disappear", **check the enum's git history before blaming the asset** — the fix is to remap the stored integers, never to reorder the enum back. Any new inspector-authored enum in this project should assign its numbers explicitly from the start.
...
`Assets/Scripts/UI/Utility/FlipnoteStyleAnimation/` cycles a UGUI `Image` through hand-drawn sprite frames. It shares **nothing** with `DOTweenAnimationPlayer/` and has no DOTween dependency — it only ever writes `Image.sprite`, so DOTween can move, scale and fade the same object at the same time. **Read that folder's `README.md` before changing it.**

- **`UIFlipbookButton` used to exist and was a byte-identical duplicate of `UIFlipbookSelectable`.** `7e7af24` renamed the file, `b240fa7` accidentally re-added the old one, and the two then coexisted for the rest of the jam with the *dead* copy owning the shared state enum. Deleted. If a class here ever looks duplicated again, check `git log --name-status` before assuming both are wanted.
- **A clip that merely ran out is not the same as one that was `Stop()`ped, and `OnEnable` has to tell them apart.** The old code tested `current == null`, which is false in both cases, so a `Once` clip played exactly once per scene load and a panel reshown later sat frozen on its last drawing. `stoppedExplicitly` is the discriminator: `Stop()` sets it, completion does not, `Play()` clears it. Verified by invoking `OnEnable` through reflection — **edit mode does not deliver `Awake`/`OnEnable` to a component without `[ExecuteAlways]`**, so a probe that toggles `SetActive` tests nothing and silently "passes".
- **`LoopMode` replaced a serialized `bool Loop`, and Unity cannot convert between them.** Every pre-existing clip would have come back as `Once`. `MigrateLoop()` reads the legacy bool once, using the older `defaultsFilled` flag to tell authored data from a zero-filled new element. Both hidden fields must stay. The same trap as the enum-renumbering one below: silent, no error, looks like lost data.
- **Randomised frame timing is rolled per pass on the *player*, never on the clip.** A `UIFlipbookClipAsset` can drive hundreds of objects, and rolling on the clip would give them all identical timing — which is the exact thing randomising was meant to prevent. `RandomStartFrame` lives on the player for the same reason.
- **Edit-mode preview writes `Image.sprite` on real scene objects**, so `UIFlipbookPreview` captures the sprite before anything moves, restores it on stop, records one `Undo` entry as a backstop, and suppresses `OnCompleted` so a preview cannot run game code. It ends itself on selection change, play-mode change and assembly reload — static state does not survive the last two, which would otherwise strand the sprite mid-animation. Only one preview runs at a time; that is what guarantees the captured sprite belongs to the object being restored.
- **Unity's `[Range]` clamps typed input as well as the slider**, so it cannot express "slider stops at 12 but you may type 24". `UIFlipbookClipDrawer.DrawFPS` draws the slider and the field separately and only adopts the slider's value when it actually moved — otherwise a typed 30 is slammed back to 12 on the next repaint.
- `GetPropertyHeight` is not given the inspector width, so the frame thumbnail strip is deliberately **one fixed-height row** that truncates with a `+N` label rather than a wrapping grid.
