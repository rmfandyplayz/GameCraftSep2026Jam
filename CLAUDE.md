...
- **Enums that appear on serialized fields are stored as integers — only ever append to them.** Inserting or reordering a value silently repoints every asset already authored against it, with no error and no warning. **This has already cost this project once**: `7e7af24` alphabetised `UIAnimationStepType` and deleted the append-only comment, which turned the 16 authored `Scale` steps on the main-menu buttons (`Type: 2`) into `GraphicAlpha` steps that faded the buttons to invisible. Repaired in the prefab by rewriting those 16 to `Type: 12`; the comment is back on the enum and now says why. The symptom looks like lost references or a bad prefab conversion, so if animations "disappear", **check the enum's git history before blaming the asset** — the fix is to remap the stored integers, never to reorder the enum back.
